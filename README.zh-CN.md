...
Skill 的真正价值在于把布局算术写成了硬约束,让生成稳定可靠:扇出/汇入的 branch-bar 模式、边界嵌套与留白规则、半透明填充下的不透明遮蔽、图例摆放、无缝循环的周期约束、以及让光点"钻进"节点而非滑过节点的绘制顺序。交付前用自带的确定性校验脚本(`scripts/check_diagram.py`,纯标准库 Python;环境有 `python3` 时启用,否则退回人工清单)复核——框重叠、连线穿框、动画循环断裂、坐标越界——发现问题先修复再交付。无论哪条路,**产物本身始终零依赖**:一个 HTML 文件,无库、无构建。
...
│   ├── scripts/
│   │   └── check_diagram.py              # 确定性结构校验脚本(纯标准库;可选启用)
│   └── resources/
