...
- **Legend subgraphs**: a subgraph that is itself a hand-drawn legend (named 图例/Legend/Key, containing only swatch-explainer nodes) is NOT rendered as a boundary — merge its entries into dashmotion's own legend. The subgraph title may be omitted; every entry must appear.
